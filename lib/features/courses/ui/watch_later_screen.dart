  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Row(
          children: [
            Icon(Icons.bookmark_rounded, color: AppColors.accentNeon),
            SizedBox(width: 10),
            Text('Watch Later', style: TextStyle(fontWeight: FontWeight.bold)),
          ],
        ),
      ...
      body: Consumer<CoursesProvider>(
        builder: (context, provider, child) {
          ...
          final watchLaterCourses = provider.courses
              .where((c) => provider.watchLaterCourseIds.contains(c.id))
              .toList();

          if (watchLaterCourses.isEmpty) {
            return Center(
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  const Icon(Icons.bookmark_border_rounded, size: 80, color: Colors.white12),
                  const SizedBox(height: 20),
                  Text('No saved courses yet.',
                      style: TextStyle(color: textColor.withOpacity(0.5), fontSize: 16)),
                  const SizedBox(height: 10),
                  Text('Tap 🔖 on any course to watch it later.',
          ...
          return ListView.builder(
            ...
            itemCount: watchLaterCourses.length,
            itemBuilder: (context, index) {
              final course = watchLaterCourses[index];
              final locale = Provider.of<LocaleProvider>(context).locale.languageCode;
              final isWL = provider.watchLaterCourseIds.contains(course.id);
                      ...
                      children: [
                        ...
                        Padding(
                          ...
                          child: GestureDetector(
                            onTap: () => provider.toggleWatchLater(course.id),
                            child: Icon(
                              isWL ? Icons.bookmark_rounded : Icons.bookmark_border_rounded,
